...
export default function Settings() {
  ...
  const fetchClients = async () => {
    try {
      const response = await axios.get("http://localhost:8000/viewAllClients");
  ...
  const fetchClientTeam = async (orgId: string) => {
    try {
      const response = await axios.post(
        "http://localhost:8000/getClientTeam",
  ...
  const handleAddClientMember = async (data: any) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/addClientTeamMember",
        formData
      );
  ...
  const handleDeleteMember = async (user_id: any) => {
    try {
      const response = await axios.delete(
        `http://localhost:8000/deleteClientTeamMember/${user_id}`
      );
      if (response) {
  ...
  useEffect(() => {
    const fetchSuperAdminDetails = async () => {
      try {
        const response = await axios.get('http://localhost:8000/superadmin/details', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
